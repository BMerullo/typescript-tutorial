...
const PokedexList: React.FC<PokedexListProps> = ({ pokedexList }) => {
  return (
    <>
      <section className="p-9  md:p-20 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {pokedexList.map((pokemon, index) => (
          <button
            className="m-2 bg-blue-800 hover:bg-yellow-500  md:text-xl p-8  rounded-lg"
            key={index}
          >
